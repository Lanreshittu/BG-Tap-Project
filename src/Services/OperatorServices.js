...
//  Description
//  Checks that the email provided is for a registerted user already
...
//  Description
//  Checks that the email provided isn't for a registerted operator already
...
const checkDuplicateEmail = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const conn = await pool.connect();
      const sql = "SELECT email from operators;";
      const result = await conn.query(sql);
      const rows = result.rows;
      conn.release();
...
const validateOperatorData = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const confirmEmail = await emailValidation(req);

      if (confirmEmail) {
        if (await checkDuplicateEmail(req)) {
          reject("You're a registered operator");
        } else {
          resolve(true);
        }
      } else {
        reject("Put in your registered user email");
      }
    } catch (error) {
      reject(error);
    }
...
const operatorNin = (nin) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const foundNINQuery = "SELECT * FROM operatorsprofile WHERE nin = $1";
...
const validateOperatorProfileData = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
